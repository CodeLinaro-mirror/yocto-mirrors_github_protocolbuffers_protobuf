...
for file in yaml_files:
  with open(file, 'r') as f:
    ...
    for job in jobs:
      ...
      for step in steps:
        if continuous_condition and 'if' not in step:
          raise ValueError(
              'Step %s in job %s does not check the continuous-run condition'
              % (step['name'], job)
          )
        elif not continuous_condition and 'if' not in step:
          continue
        elif continuous_condition and 'continuous-run' not in step['if']:
          raise ValueError(
              'Step %s in job %s does not check the continuous-run condition'
              % (step['name'], job)
          )
        elif not continuous_condition and 'continuous-run' in step['if']:
          raise ValueError(
