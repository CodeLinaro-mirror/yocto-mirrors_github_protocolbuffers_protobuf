...
std::unique_ptr<ForkPipeRunner> MakeRunner(
    const std::string& script,
    absl::Duration shutdown_grace_period =
        ForkPipeRunner::kDefaultShutdownGracePeriod,
    absl::Duration read_timeout = ForkPipeRunner::kDefaultReadTimeout) {
  const std::vector<std::string> args = {"-c", script};
  return std::make_unique<ForkPipeRunner>(kShell, args, shutdown_grace_period,
                                          read_timeout);
}
...
TEST(ForkPipeRunnerTest, PipesAreNotInheritedByAnotherTestee) {
  ...
  auto first = MakeRunner("exec cat", kGrace);
...
TEST(ForkPipeRunnerTest, StubbornTesteeIsKilled) {
  ...
  auto runner =
      MakeRunner("trap '' PIPE; cat; while :; do sleep 1; done", kGrace);
  EXPECT_EQ(runner->RunTest("t", "abc"), "abc");
...
TEST(ForkPipeRunnerTest, StubbornTesteesGrandchildIsKilledToo) {
  ...
  constexpr absl::Duration kGrace = absl::Milliseconds(100);
  auto runner = MakeRunner("sleep 1000 & cat; exec sleep 1000", kGrace);
  EXPECT_EQ(runner->RunTest("t", "abc"), "abc");
  pipe.CloseWriteEnd();
...
TEST(ForkPipeRunnerTest, HungTesteeTimesOutAndIsKilled) {
  ...
  auto runner =
      MakeRunner("trap '' QUIT; head -c 7 >/dev/null; exec sleep 1000",
                 ForkPipeRunner::kDefaultShutdownGracePeriod, kReadTimeout);
...
TEST(ForkPipeRunnerTest, HungTesteeKilledBySigquitIsReportedAsSuch) {
  ...
  constexpr absl::Duration kReadTimeout = absl::Milliseconds(200);
  auto runner =
      MakeRunner("head -c 7 >/dev/null; exec sleep 1000",
                 ForkPipeRunner::kDefaultShutdownGracePeriod, kReadTimeout);
