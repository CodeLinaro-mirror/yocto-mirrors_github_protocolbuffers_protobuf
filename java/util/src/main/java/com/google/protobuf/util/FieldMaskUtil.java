...
public final class FieldMaskUtil {
  ...
  private static String getFieldNameOrThrow(Descriptor desc, int fieldNumber) {
    FieldDescriptor field = desc.findFieldByNumber(fieldNumber);
    if (field == null) {
      throw new IllegalArgumentException(
          String.format("%s is not a valid field number for %s.", fieldNumber, desc.getFullName()));
    }
  ...
  public static FieldMask fromFieldNumbers(Class<? extends Message> type, int... fieldNumbers) {
    ...
    for (int fieldNumber : fieldNumbers) {
      builder.addPaths(getFieldNameOrThrow(descriptor, fieldNumber));
    }
    return builder.build();
  }
