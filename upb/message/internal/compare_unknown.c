...
static upb_UnknownFields* upb_UnknownFields_Build_From_Buffer(
  ...
  while (!upb_EpsCopyInputStream_IsDone(&ctx->stream, &ptr)) {
    ...
    switch (wire_type) {
      ...
      case kUpb_WireType_StartGroup:
        ...
        field->data.group = upb_UnknownFields_Build_From_Buffer(ctx, &ptr);
        ctx->depth++;
        break;
      default:
        UPB_UNREACHABLE();
...
upb_UnknownCompareResult UPB_PRIVATE(_upb_Message_UnknownFieldsAreEqual)(
  ...
  while (upb_Message_NextUnknown(msg2, &view2, &iter)) {
    size2 += view2.size;
  }
