...
//
// If kUpb_ConvertOption_Alias is passed, strings and unknown fields are
// aliased from the input message. Additionally, if the destination message,
// or any of its submessages, uses the same minitable as the matching part
// of the source (determined by pointer identity), the destination will
// alias the source's memory (shallow copy) instead of performing a deep copy.
//
...
// registry allows it).
//
// Options for upb_Message_Convert()
enum {
  kUpb_ConvertOption_Copy = 0,
  kUpb_ConvertOption_Alias = 1 << 0,
};
