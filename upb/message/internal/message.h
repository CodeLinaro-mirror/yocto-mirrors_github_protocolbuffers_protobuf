...
UPB_FORCEINLINE void _upb_Message_AlignedMemsetZero(void* dst, size_t size) {
  ...
  __asm__(
      ...
      // Loop for size >= 16.
      // In each iteration, we zero 16 bytes from the front (dst) and 16 bytes
      // from the back (end). These regions may overlap, which is OK; doing it
      // this way lets us process two chunks per loop iteration.
      "1:\n\t"
