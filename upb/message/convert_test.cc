...
class ConvertTest : public testing::TestWithParam<int> {};

INSTANTIATE_TEST_SUITE_P(ConvertOptions, ConvertTest,
                         testing::Values(kUpb_ConvertOption_Copy,
...
TEST_P(ConvertTest, Identity) {
  ...
  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), mt, mt, nullptr, GetParam(), arena.ptr());
...
TEST_P(ConvertTest, AliasSubMessage) {
  upb::Arena arena;
  protobuf_test_messages_proto3_TestAllTypesProto3* msg =
      protobuf_test_messages_proto3_TestAllTypesProto3_new(arena.ptr());
...
TEST_P(ConvertTest, ConvertField_UnknownsConversion) {
  ...
  *ptr++ = (char)0x06;
  size_t len = ptr - buf;
  UPB_PRIVATE(_upb_Message_AddUnknown)(UPB_UPCAST(msg), buf, len, arena.ptr(),
  ...
  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), mt, dst_mt, nullptr, GetParam(), arena.ptr());
  EXPECT_NE(dst_msg, nullptr);

  // Since everything is demoted to unknowns, we can inspect the unknown fields
  // to verify whether strings and unknowns were aliased.
  size_t iter = kUpb_Message_UnknownBegin;
  upb_StringView data;
  upb_Message_NextUnknown(dst_msg, &data, &iter);

  // When aliasing is not requested (kUpb_ConvertOption_Copy), deep copy should
  // allocate new memory for the encoded unknown fields.
  // When aliasing is requested, they are aliased.
  // Note: the original `buf` is added to `msg` via `upb_Message_AddUnknown`
  // with `kUpb_AddUnknown_Copy`, so `msg` allocates its own buffer for it.
  // Therefore, we must compare `dst_msg`'s unknowns against `msg`'s unknowns
  // to check aliasing, not against `buf`.
  size_t src_iter = kUpb_Message_UnknownBegin;
  upb_StringView src_data;
  upb_Message_NextUnknown(UPB_UPCAST(msg), &src_data, &src_iter);

  if (GetParam() & kUpb_ConvertOption_Alias) {
...
TEST_P(ConvertTest, UnknownPromotion) {
  ...
  *ptr++ = (char)0x06;
  ...
  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      msg_empty, empty_mt, mt, nullptr, GetParam(), arena.ptr());
  EXPECT_NE(dst_msg, nullptr);
  const protobuf_test_messages_proto3_TestAllTypesProto3* dst =
      (const protobuf_test_messages_proto3_TestAllTypesProto3*)dst_msg;
...
TEST_P(ConvertTest, Demotion) {
  ...
  const upb_Message* dst = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), TEST_MT, empty_mt, nullptr, GetParam(), arena.ptr());
...
TEST_P(ConvertTest, DeepConvertMapMessage) {
  ...
  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), src_mt, dst_mt, nullptr, GetParam(), arena.ptr());
  EXPECT_NE(dst_msg, nullptr);
...
TEST_P(ConvertTest, ConvertField_SingularMessageConversion) {
  upb::Arena arena;
  upb_test_convert_MessageWithMsg* msg =
      upb_test_convert_MessageWithMsg_new(arena.ptr());
  upb_test_convert_MessageWithInt32* sub =
  ...
  const upb_MiniTable* src_mt = &upb__test__convert__MessageWithMsg_msg_init;
  const upb_MiniTable* dst_mt = &upb__test__convert__MessageWithMsg_msg_init;

  // Since dst_mt == src_mt, but we want to force conversion to test field-level
  // options instead of the top-level identity short-circuit, we'll provide a
  // dummy extension registry to bypass the short-circuit.
  upb_ExtensionRegistry* extreg = upb_ExtensionRegistry_New(arena.ptr());

  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), src_mt, dst_mt, extreg, GetParam(), arena.ptr());
  EXPECT_NE(dst_msg, nullptr);

  const upb_test_convert_MessageWithMsg* dst =
      (const upb_test_convert_MessageWithMsg*)dst_msg;
  const upb_test_convert_MessageWithInt32* dst_sub =
      upb_test_convert_MessageWithMsg_msg(dst);
  EXPECT_NE(dst_sub, nullptr);
...
TEST_P(ConvertTest, ConvertField_ArrayMessageConversion) {
  upb::Arena arena;
  ...
  const upb_Message* dst_msg = UPB_PRIVATE(_upb_Message_Convert)(
      UPB_UPCAST(msg), mt, mt, extreg, GetParam(), arena.ptr());
  EXPECT_NE(dst_msg, nullptr);
