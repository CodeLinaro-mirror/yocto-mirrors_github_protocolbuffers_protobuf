...
static void upb_Message_EncodeExtensionAsUnknown(
    upb_Converter* c, upb_encstate* e, upb_Message* dst,
    const upb_MiniTable* dst_mt, const upb_MiniTableExtension* ext,
    upb_MessageValue val, int depth, upb_ErrorHandler* err) {
  char* buf;
  size_t size;
  ...
  UPB_PRIVATE(_upb_Encode_Extension)(e, ext, val, is_message_set, &buf, &size,
                                     encode_options);
  int add_options = (c->options & kUpb_ConvertOption_Alias)
...
static bool upb_Message_ConvertField(upb_Converter* c, upb_Message* dst,
    ...
    if (upb_MiniTableField_IsScalar(dst_f)) {
      ...
      if (dst_sub_mt == src_sub_mt) {
        if (c->options & kUpb_ConvertOption_Alias) {
          ...
          upb_Message* dst_sub =
              upb_Message_DeepClone(src_sub, dst_sub_mt, c->arena);
          if (!dst_sub) {
            upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
          }
          upb_Message_SetMessage(dst, dst_f, dst_sub);
        }
        return true;
      }
  ...
  if (upb_MiniTableField_IsMap(dst_f)) {
    ...
    if (c->options & kUpb_ConvertOption_Alias) {
      ...
      upb_Map* dst_map = upb_Map_DeepClone(src_map, key_type, val_type,
                                           dst_entry_mt, c->arena);
      if (src_map && !dst_map) {
        upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
      }
      upb_Message_SetBaseField(dst, dst_f, &dst_map);
    ...
    if (c->options & kUpb_ConvertOption_Alias) {
      ...
      const upb_MiniTable* dst_sub_mt = upb_MiniTable_SubMessage(dst_f);
      upb_Array* dst_arr = upb_Array_DeepClone(
          src_arr, upb_MiniTableField_CType(dst_f), dst_sub_mt, c->arena);
      if (src_arr && !dst_arr) {
        upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
      }
      upb_Message_SetBaseField(dst, dst_f, &dst_arr);
    ...
    if (c->options & kUpb_ConvertOption_Alias) {
      ...
      char* new_str = upb_Arena_Malloc(c->arena, src_str.size);
      if (src_str.size > 0 && !new_str) {
        upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
      }
      if (src_str.size > 0) {
        memcpy(new_str, src_str.data, src_str.size);
      }
      upb_StringView dst_str =
          upb_StringView_FromDataAndSize(new_str, src_str.size);
      UPB_PRIVATE(_upb_MiniTableField_DataCopy)(
          dst_f, UPB_PRIVATE(_upb_Message_MutableDataPtr)(dst, dst_f),
          &dst_str);
    }
  ...
  while (upb_Message_NextExtension(src, &ext, &val, &iter)) {
    ...
    if (dst_f) {
      ...
      if (upb_MiniTableField_CType(dst_f) != upb_MiniTableField_CType(src_f) ||
        ...
        // destination message.
        upb_Message_EncodeExtensionAsUnknown(c, &c->encoder, dst, dst_mt, ext,
                                             val, depth, &c->err);
        ...
        if (upb_MiniTableField_IsArray(dst_f)) {
          if (dst_sub_mt != src_sub_mt) {
            ...
            if (c->options & kUpb_ConvertOption_Alias) {
              ...
              upb_Array* dst_arr = upb_Array_DeepClone(
                  val.array_val, upb_MiniTableField_CType(dst_f), dst_sub_mt,
                  c->arena);
              if (!dst_arr) {
          ...
          if (c->options & kUpb_ConvertOption_Alias) {
            ...
            upb_Message* dst_sub =
                upb_Message_DeepClone(val.msg_val, dst_sub_mt, c->arena);
            if (!dst_sub) {
              upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
            }
            upb_MessageValue valid_val;
        ...
        if (c->options & kUpb_ConvertOption_Alias) {
          ...
          upb_Array* dst_arr = upb_Array_DeepClone(
              val.array_val, upb_MiniTableField_CType(dst_f), NULL, c->arena);
          if (!dst_arr) {
            upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
          }
          upb_MessageValue valid_val;
        ...
        if (c->options & kUpb_ConvertOption_Alias) {
          ...
          upb_StringView src_str = val.str_val;
          char* new_str = upb_Arena_Malloc(c->arena, src_str.size);
          if (src_str.size > 0 && !new_str) {
            upb_ErrorHandler_ThrowError(&c->err, kUpb_ErrorCode_OutOfMemory);
          }
...
static void upb_Message_ConvertInternal(upb_Converter* c, upb_Message* dst,
  ...
  while (upb_Message_NextUnknown(src, &data, &iter)) {
    int decode_options = (c->options & kUpb_ConvertOption_Alias)
                             ? kUpb_DecodeOption_AliasString
                             : 0;
    decode_options = upb_Decode_LimitDepth(decode_options, depth);
