...
UPB_INLINE uint8_t upb_DecodeFastData_GetMask(uint64_t data2) {
  return data2 >> 56;
}

UPB_INLINE uint64_t upb_DecodeFastData_PackMask(uint8_t mask, uint64_t data2) {
  return ((uint64_t)mask << 56) | (data2 & 0x00ffffffffffffffULL);
}

UPB_INLINE uint16_t upb_DecodeFastData_GetOriginalTag(uint64_t data2) {
  return data2 & 0xffff;
}

UPB_INLINE uint64_t upb_DecodeFastData_PackOriginalTag(uint64_t data2,
                                                       uint16_t tag) {
