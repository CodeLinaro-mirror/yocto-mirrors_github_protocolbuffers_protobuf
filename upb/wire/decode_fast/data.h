...
// - `index` is the index of the field in the mini table's fields array, or the
//   8 byte shifted submessage offset from the fields array start into the subs
//   array (or 0 if neither is needed).
...
UPB_INLINE uint8_t upb_DecodeFastData_GetIndex(uint64_t data) {
  return data >> 16;
}
