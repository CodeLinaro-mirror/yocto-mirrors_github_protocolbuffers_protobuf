...
static const char* _upb_Decoder_DecodeEmptyMessage(upb_Decoder* d,
                                                   const char* ptr,
                                                   upb_Message* msg) {
  const char* saved_capture = d->input.capture_start;
  upb_EpsCopyInputStream_StartCapture(&d->input, ptr);
  while (!upb_EpsCopyInputStream_IsDone(EPS(d), &ptr)) {
    uint32_t tag;
    ptr = upb_WireReader_ReadTag(ptr, &tag, EPS(d));
    if (ptr == NULL) break;
  ...
  d->input.capture_start = saved_capture;
  return ptr;
}
