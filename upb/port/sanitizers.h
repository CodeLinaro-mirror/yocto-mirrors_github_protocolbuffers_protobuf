...
// UPB_ARENA_SIZE_HACK depends on this struct having size 0 or 1.
typedef struct {
#if UPB_HWASAN || UPB_TSAN
  uint8_t state;
#else
  uint8_t nothing[0];  // Try to be zero-sized, though on MSVC it doesn't work.
#endif
} upb_Xsan;
...
// pointer.
//
// The `addr` must be aligned to the malloc alignment.  Size may be unaligned,
