...
UPB_INLINE upb_StringView json_enum_name(const upb_EnumValueDef* ev,
  ...
  const google_protobuf_EnumValueOptions* opts = upb_EnumValueDef_Options(ev);
  if (!opts) {
    return res;
  }
  if (!symtab) {
  ...
  const upb_Message* opts_msg = (const upb_Message*)opts;
  if (!upb_Message_HasFieldByDef(opts_msg, ext_field)) {
    return res;
  }
  upb_MessageValue ext_val = upb_Message_GetFieldByDef(opts_msg, ext_field);
