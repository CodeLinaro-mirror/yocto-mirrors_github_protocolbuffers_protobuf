...
bool ValidateCcNamespace(const FileDescriptor* file, std::string* error) {
  if (file->options().GetExtension(::pb::file::cpp).has_namespace_()) {
    ...
    // static const LazyRE2 re = {"[a-zA-Z0-9_:]"}; !RE2::FullMatch(letter, *re)
    for (auto& letter : letters) {
      if (letter != "_" && letter != ":" &&
          !absl::ascii_isalnum(static_cast<unsigned char>(letter[0]))) {
        *error = absl::StrCat("Namespace contains invalid character: ", letter);
        return false;
      }
    }
  }
