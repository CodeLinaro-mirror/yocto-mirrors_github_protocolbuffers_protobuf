...
void CheckAnnotations(
    const atu::ExpectedOutput& output, const std::vector<int>& path,
    absl::Span<const std::pair<std::string, Semantic>> expected_contents) {
  for (const auto& expected : expected_contents) {
    CheckAnnotation(output, path, expected.first, expected.second);
  }
}
...
TEST_F(KotlinMetadataTest, CapturesFooOrNull) {
  ...
  CheckAnnotations(output, {kMessageTypeFieldNumber, 0, kFieldFieldNumber, 0},
