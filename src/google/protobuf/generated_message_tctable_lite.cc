...
PROTOBUF_ALWAYS_INLINE void SetCachedHasBit(uint64_t& cached_hasbits,
                                            uint8_t hasbit_idx) {
...
PROTOBUF_ALWAYS_INLINE const char* TcParser::SingularParseMessageAuxImpl(
  ...
  const auto* class_data = inner_table->class_data;
#else
  const auto* globals = aux.message_globals();
  const auto* inner_table = aux_is_table
                                ? MessageGlobalsBase::ToParseTableBase(globals)
                                : aux.message_default()->GetTcParseTable();
  const auto* class_data = aux_is_table
                               ? MessageGlobalsBase::GetClassData(globals)
                               : inner_table->class_data;
#endif
...
TcParser::TableAndClassData TcParser::GetTableAndClassDataFromAux(
    uint16_t type_card, TcParseTableBase::FieldAux aux) {
  uint16_t tv = type_card & field_layout::kTvMask;
  ...
  if (ABSL_PREDICT_TRUE(tv == field_layout::kTvTable)) {
#ifndef PROTOBUF_MESSAGE_GLOBALS
    inner_table = aux.table_ptr();
    class_data = inner_table->class_data;
#else
    const auto* globals = aux.message_globals();
    inner_table = MessageGlobalsBase::ToParseTableBase(globals);
    class_data = MessageGlobalsBase::GetClassData(globals);
#endif
  } else {
    ...
    const MessageLite* prototype = tv == field_layout::kTvDefault
                                       ? aux.message_default()
                                       : aux.message_default_weak();
    inner_table = prototype->GetTcParseTable();
    class_data = inner_table->class_data;
  }
  return {inner_table, class_data};
}
...
PROTOBUF_NOINLINE const char* TcParser::MpMessage(PROTOBUF_TC_PARAM_DECL) {
  ...
  const auto aux = *table->field_aux(&entry);
  auto res = GetTableAndClassDataFromAux(type_card, aux);
  const TcParseTableBase* inner_table = res.table;
  const ClassData* class_data = res.class_data;
