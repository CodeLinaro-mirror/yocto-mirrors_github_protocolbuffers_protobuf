...
class PROTOBUF_EXPORT SerialArena {
 ...
 private:
  ...
  // bytes.
  char* ArbitraryInternalPointerForInit() {
    if constexpr (alignof(SerialArena) >= 8) {
      return reinterpret_cast<char*>(this);
    } else {
      // We need to make sure it is aligned even in 32-bit platforms.
      return ArenaAlignAs(8).Ceil(reinterpret_cast<char*>(this));
    }
  }
