...
bool IsViewValidUTF8(absl::string_view fragment, std::string& leftover) {
  if (size_t leftover_size = leftover.size(); leftover_size != 0) {
    ABSL_DCHECK_LT(leftover_size, UTFmax);

    // Copy into the leftover buffer until it has UTFmax bytes, and match code
    // points within that buffer, removing them from the prefix of the next
    // chunk.
    const size_t fill_size = UTFmax - leftover_size;
    if (fragment.size() < fill_size) {
      ...
      leftover = leftover.substr(utf8_range::SpanStructurallyValid(leftover));
      return true;
    }
    ...
    const int leftover_valid = utf8_range::SpanStructurallyValid(leftover);
    if (leftover_valid == 0) {
      return false;
    }
