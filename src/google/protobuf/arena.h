...
//
// This class is intentionally left empty, and types with a different
// representation on an arena will specialize it and define the fields commented
// below.
template <typename T>
struct FieldArenaRep {
  // The type of the representation of the field on an arena.
  // using Type = ...;

  // Returns a pointer to the wrapped type `T`. For types which have a different
  // representation when allocated on an arena/individually, this is a way to
  // access the wrapped type.
  // static inline constexpr T* Get(Type* arena_rep);
};
